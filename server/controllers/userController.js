...
const requestCreator = async (req, res) => {
    console.log("🚀 תהליך בקשת יוצר התחיל עבור משתמש:", req.user.email);
    try {
        const currentUserId = req.user._id || req.user.userId;
        ...
        console.log("✅ משתמש עודכן במסד הנתונים, מנסה לשלוח מייל...");
        ...
        const mailOptions = {
            ...
            subject: '👑 בקשה חדשה להצטרפות כיוצר תוכן',
            html: `<h1>בקשה חדשה מ-${updatedUser.userName}</h1>`
        };

        transporter.sendMail(mailOptions, (error, info) => {
            if (error) {
                console.error("❌ שגיאת Nodemailer:", error);
            } else {
        ...
        console.error("🔥 קריסה ב-requestCreator:", ex);
...
const updateUserRole = async (req, res) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(userIdToUpdate, { role: role, requestedCreator: false }, { new: true });
