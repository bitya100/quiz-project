...
const register = async (req, res) => {
    try {
        // שים לב: הורדנו מכאן את ה-validateUser! ה-Middleware עשה את זה לפני שנכנסנו לפה.
...
// ... כל שאר הפונקציות בקובץ נשארות בדיוק אותו דבר (login, getProfile, requestCreator, getAllUsers, updateUserRole, deleteUser) ...
// (אל תשנה את שאר הפונקציות, הן מצוינות. רק ה-register השתנה)

const login = async (req, res) => { /* ללא שינוי */
    try {
    ...
    } catch (ex) { res.status(500).send(ex.message || 'שגיאה בתהליך ההתחברות'); }
};

const getProfile = async (req, res) => { /* ללא שינוי */
    try {
        const currentUserId = req.user._id || req.user.userId;
        const user = await User.findById(currentUserId).select('-password');
        if (!user) return res.status(404).send('משתמש לא נמצא');
        const freshToken = jwt.sign( { _id: user._id, userId: user._id, role: user.role, userName: user.userName }, process.env.JWT_SECRET || 'secret', { expiresIn: '7d' } );
...
const requestCreator = async (req, res) => { /* ללא שינוי */
    try {
        const currentUserId = req.user._id || req.user.userId;
...
const getAllUsers = async (req, res) => { /* ללא שינוי */
    try {
...
const updateUserRole = async (req, res) => { /* ללא שינוי */
    try {
        if (req.user.role !== 'admin') return res.status(403).send("גישה נדחתה: מנהלים בלבד");
        const currentUserId = req.user._id || req.user.userId;
        ...
        if (role === 'admin' && userToUpdate.role !== 'admin' && userToUpdate.requestedCreator !== true && !isSuperAdmin) return res.status(400).send('לא ניתן למנות למנהל משתמש שלא הגיש בקשה לכך במערכת.');
        if (role === 'user') {
            const adminCount = await User.countDocuments({ role: 'admin' });
            if (userToUpdate.role === 'admin' && adminCount <= 1) return res.status(400).send('לא ניתן להסיר את המנהל האחרון במערכת.');
        }
        userToUpdate.role = role;
        userToUpdate.requestedCreator = false; 
        await userToUpdate.save();
        res.json({ message: `המשתמש ${userToUpdate.userName} הוא כעת ${role}`, user: userToUpdate });
    } catch (ex) { res.status(500).send('שגיאה בעדכון התפקיד'); }
};

const deleteUser = async (req, res) => { /* ללא שינוי */
    try {
        const currentUserId = req.user._id || req.user.userId;
        const requestingUser = await User.findById(currentUserId);
        if (requestingUser.email !== SUPER_ADMIN_EMAIL) return res.status(403).send("גישה נדחתה: רק מנהל-על מורשה למחוק משתמשים");
