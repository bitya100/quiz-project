...
// 🚀 החיבור המקצועי ל-Resend דרך פורט 587 - פתוח ב-100% בשרתי ענן ולא נחסם לעולם
const transporter = nodemailer.createTransport({
    host: 'smtp.resend.com',
    port: 587,
    secure: false, 
    auth: {
        user: 'resend', // זה תמיד נשאר המילה 'resend' בכתב קטן! לא המייל שלך
        pass: process.env.RESEND_API_KEY // ה-API Key שהגדרנו ברנדר
    }
});
...
const forgotPassword = async (req, res) => {
    try {
        ...
        const mailOptions = {
            // 🚀 חובה להשתמש בכתובת הזו ב-Resend כל עוד אין לך דומיין פרטי משלך קנוי
